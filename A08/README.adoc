...
It allocates 0 byte. This is because whenever a new request for memory appears, we will fetch from the only free block on the free list.
This means that keep allocating and freeing the same block of memory, which in the end makes us use 0 byte. 
